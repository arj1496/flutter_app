...
class EventWebService {
  ...
  Future<Map<String , dynamic>> addOrUpdateEvent( Event event ) async {
    ...
    Response response = await post (
        finalurl , headers: headers , body: eventMap );
    var data;
  ...
  Map<String , String> getEventFrom( Event event ) {
    ...
    if (event.eventParticipant != null) {
      List<Map<String , String>> test = prepareParticipantmap (
    ...
    if (event.personalParticipant != null) {
      List<Map<String , List<int>>> test = preparePersonalParticipantmap (
          event.personalParticipant );
  ...
  Future<dynamic> getParticipants( Event event ) async {
    ...
    final finalurl = urlUtils.getParticpantUrl ( ) + "?eventId=" +
        event.id.toString ( );
    var response = await http.get ( finalurl , headers: headers );
    if (response.statusCode == 200) {
      final data = json.decode ( response.body );
